...
Vamos criar um gráfico de barras da variável expectativa de vida. Nesse caso, vamos pegar os 25 países da Ásia com maior taxa de expectativa de vida. 
...
```{r barras3, echo=TRUE, message=FALSE, warning=FALSE,fig.align='center', fig.height=4, fig.width=4}
library(forcats)
base %>% 
  filter(continent == "Asia") %>% 
  top_n(n = 10, wt = lifeExp) %>% 
...
Depois de vermos os boxplots, fica claro que é interessante visualizar o histograma por continente também. 

```{r densidade_customizada2, echo=TRUE, fig.align='center', fig.height=5.5, fig.width=6.5, message=FALSE, warning=FALSE}

ggplot(base,aes(x=lifeExp, fill = continent)) + geom_density(aes(alpha = 1)) +
labs(title = "Histograma da expectativa de vida", 
     x = "Expectativa de vida", y = "Frequência") + theme_bw()
```
